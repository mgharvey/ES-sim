essim_pglsslope <- function(phy, trait, a = 0.5, nsim = 1000, is) {
	...
	dframe <- data.frame(names(trait), trait, is[as.vector(names(trait))])
	...
	sim.res <- sapply(vars, function(x) summary(pgls(invsplits ~ x, data = sim.data))$coefficients)
	sim.r <- sim.res[2,]
	sim.p <- sim.res[8,]
