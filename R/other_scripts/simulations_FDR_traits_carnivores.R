...
nsim <- 500 # Number of traits to simulate per tree per treeset

### Simulate traits ###

# BM (uncorrelated)

treefiles <- list.files("./trees/", pattern="trees_carnivore.tre", full.names=T)
...
for(i in 1:length(treefiles)){
	...
	for(j in 1:length(trees)) {
		...
		# Trait simulations 
		sim.vals <- rnorm(length(tree$tip.label)*500, mean = 0, sd = max(branching.times(tree)))
		sims <- matrix(unlist(sim.vals), ncol=500)
...
for(i in 1:length(treefiles)){
	...
	for(j in 1:length(trees)) {
		...
		norm.vals <- rnorm(length(subclade$tip.label)*500, mean = 0, sd = max(branching.times(subclade)))
...
for(i in 1:length(treefiles)){
	...
	for(j in 1:length(trees)) {
		...
		sims <- matrix(unlist(statecols), ncol=500)		
		rownames(sims) <- tree$tip.label	
